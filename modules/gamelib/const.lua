...
-- was missing in the ported client's const.lua: enableFeature(nil) set flag 0 instead of 124,
-- which made the client not read the stackpos byte in 0x6A/0x6C = desync of the whole map
...
-- standard OTServ key (crystal-summerupdate); the ported client had its own server's key here
