-- NOTE: the modulelib Controller (version from the ported client) buckets events by phase
-- (self.currentTypeEvent) and does not hook up those registered at chunk level - the protocol
-- flags were never being set (incl. GameTileAddThingWithStackpos = map desync).
-- That is why we hook in directly via connect(), independently of the module framework.
controller = Controller:new()

-- a missing constant in const.lua = enableFeature(nil) = flag 0 silently set instead of the proper one
-- (that is how GameTileAddThingWithStackpos got lost and the whole map went out of sync) - now it screams
local rawEnableFeature = g_game.enableFeature
local rawDisableFeature = g_game.disableFeature

g_game.enableFeature = function(feature)
    if feature == nil then
        g_logger.error("[features] enableFeature(nil) - missing Game* constant in gamelib/const.lua! " .. debug.traceback("", 2))

        return
    end

    return rawEnableFeature(feature)
end

g_game.disableFeature = function(feature)
    if feature == nil then
        g_logger.error("[features] disableFeature(nil) - missing Game* constant in gamelib/const.lua! " .. debug.traceback("", 2))

        return
    end

    return rawDisableFeature(feature)
end
...
local featureEvents = {
    onClientVersionChange = function(version)
        ...
        g_logger.info(string.format(
            "[features] version %d: stackpos=%s mounts=%s quiver=%s marks=%s itemShader=%s",
            version,
            tostring(g_game.getFeature(GameTileAddThingWithStackpos)),
            tostring(g_game.getFeature(GamePlayerMounts)),
            tostring(g_game.getFeature(GameThingQuiver)),
            tostring(g_game.getFeature(GameThingMarks)),
            tostring(g_game.getFeature(GameItemShader))))
    end
}

-- Game::setClientVersion does m_features.reset() on EVERY version change, and onGameStart
-- may happen after that reset - that is why we also set the flags right before entering the game
featureEvents.onGameStart = function()
    local version = g_game.getClientVersion()

    if version and version > 0 then
        featureEvents.onClientVersionChange(version)
    end
end

connect(g_game, featureEvents)

-- if the version is already set (module loaded later than setClientVersion), apply immediately
if g_game.getClientVersion() and g_game.getClientVersion() > 0 then
    featureEvents.onClientVersionChange(g_game.getClientVersion())
end
